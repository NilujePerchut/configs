...
local function worker(args)
   ...
   pulseaudio.cmd    = args.cmd or string.format("pacmd list-sinks | sed -n -e '0,/*/d' -e '/base volume/d' -e '/volume:/p' -e '/muted:/p' -e '/device.string/p' -e '/name:/p'")
   local s = read_pipe(pulseaudio.cmd)
   pulseaudio.sink  = string.match(s, "name: <(%S+)>") or "N/A"
   ...
   function pulseaudio.update()
      ...
      volume_now.sink  = string.match(s, "name: <(%S+)>") or "N/A"
      volume_now.muted = string.match(s, "muted: (%S+)") or "N/A"
      volume_now.device_string = string.match(s, "device.string = \"(%S+)\"") or "N/A"
