...
-- {{{ Tags
tags = {
   names = { "[1:files]", "[2:code]", "[3:ssh]", "4", "5",
             "6", "7", "[8:www]", "[9:sic]"},
...
-- Send a command to audicious
local function audacious(command)
   awful.util.spawn("audacious " .. command, false)
end
...
volumewidget = lain.widgets.pulseaudio({
    settings = function()
	-- Assumes that the volume is the same between left and right
...
globalkeys = awful.util.table.join(
    ...
    -- Do jot interfere with gvim keybinding
    ...
    awful.key({ altkey, "Control" }, "m",
        function ()
            os.execute(string.format("pactl set-sink-volume %s 100%%", volumewidget.sink))
            volumewidget.update()
        end),

    -- Audacious control
    awful.key({ }, "XF86AudioPlay", function() audacious("--play") end),
    awful.key({ }, "XF86AudioPause",function() audacious("--play-pause") end),
    awful.key({ }, "XF86AudioStop", function() audacious("--stop") end),
    awful.key({ }, "XF86AudioNext", function() audacious("--fwd") end),
    awful.key({ }, "XF86AudioPrev", function() audacious("--rew") end),
    ...
    awful.key({ modkey }, "n", function () awful.util.spawn("nemo --no-desktop") end),
