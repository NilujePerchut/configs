...
Use Gruvbox or one dark because Solarized is just a disaster on emacs
#+begin_src elisp
...
Also provides a function to disable the whitespace-mode for prog modes
that do not need it (ex: org mode). ~prog-mode-hook~ is sadely executed AFTER
mode specific hooks and might reactivate whitespaces.

To avoid that, the ~niluje/turn-off-whitespace-mode~ function sets the
~whitespace-style~ to ~nil~. This will ensure that, even reactivated,
it won't be visible.
...
* TODOS
** TODO Fix the disable-whitespace method in a more elegant way.
